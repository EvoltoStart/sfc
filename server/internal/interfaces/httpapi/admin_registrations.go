...
func registerAdminRoutes(mux *http.ServeMux, svc *service.Service, handler *Handler) {
	...
	mux.HandleFunc("POST /api/v1/admin/audits/{id}/approve", middleware.RequireAdminAuth(svc, handler.approveAdminAudit))
	mux.HandleFunc("POST /api/v1/admin/audits/{id}/reject", middleware.RequireAdminAuth(svc, handler.rejectAdminAudit))
	...
	mux.HandleFunc("POST /api/v1/admin/cms/banners", middleware.RequireAdminAuth(svc, handler.createCMSBanner))
	mux.HandleFunc("PUT /api/v1/admin/cms/banners/{id}", middleware.RequireAdminAuth(svc, handler.updateCMSBanner))
	mux.HandleFunc("GET /api/v1/admin/cms/articles/{type}", middleware.RequireAdminAuth(svc, handler.getCMSArticle))
	mux.HandleFunc("PUT /api/v1/admin/cms/articles/{type}", middleware.RequireAdminAuth(svc, handler.updateCMSArticle))
}
